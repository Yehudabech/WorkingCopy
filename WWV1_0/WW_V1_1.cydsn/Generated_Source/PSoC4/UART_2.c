...
static void UART_2_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void UART_2_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            UART_2_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            UART_2_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (UART_2_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_OD_LO;
        #elif (UART_2_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_2_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_OD_LO;
        #else
        #endif /* (UART_2_RX_SDA_MOSI_PIN) */
        
        #if (UART_2_TX_SCL_MISO_PIN)
            hsiomSel[UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_PIN_DM_OD_LO;
        ...
        {
        #if (UART_2_RX_SDA_MOSI_PIN)
            hsiomSel[UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (UART_2_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_2_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (UART_2_RX_SDA_MOSI_PIN) */
        
        #if (UART_2_TX_SCL_MISO_PIN)
            hsiomSel[UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (UART_2_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_DIG_HIZ;
                pinsDm[UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= UART_2_TX_SCL_MISO_PIN_MASK;
            #endif /* (UART_2_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_STRONG;
                pinsDm[UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_PIN_DM_DIG_HIZ;
            ...
            #if (UART_2_RX_SDA_MOSI_PIN)
                pinsInBuf |= UART_2_RX_SDA_MOSI_PIN_MASK;
            #elif (UART_2_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= UART_2_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (UART_2_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (UART_2_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (UART_2_TX_SCL_MISO_PIN)
                hsiomSel[UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [UART_2_TX_SCL_MISO_PIN_INDEX] = UART_2_PIN_DM_OD_LO;
            #endif /* (UART_2_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (UART_2_RX_SDA_MOSI_PIN)
                    hsiomSel[UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_2_RX_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_DIG_HIZ;
                #elif (UART_2_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_2_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_2_PIN_DM_DIG_HIZ;
                #else
                #endif /* (UART_2_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        UART_2_SET_HSIOM_SEL(UART_2_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       UART_2_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[UART_2_RX_SDA_MOSI_PIN_INDEX]);

        UART_2_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[UART_2_RX_SDA_MOSI_PIN_INDEX]);

        #if (!UART_2_CY_SCBIP_V1)
            UART_2_SET_INP_DIS(UART_2_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         UART_2_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & UART_2_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!UART_2_CY_SCBIP_V1) */
        ...
        UART_2_SET_HSIOM_SEL(UART_2_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        UART_2_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[UART_2_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        UART_2_SET_INP_DIS(UART_2_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     UART_2_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & UART_2_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        UART_2_SET_INCFG_TYPE(UART_2_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        UART_2_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (UART_2_TX_SCL_MISO_PIN)
        UART_2_SET_HSIOM_SEL(UART_2_TX_SCL_MISO_HSIOM_REG,
                                       UART_2_TX_SCL_MISO_HSIOM_MASK,
                                       UART_2_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[UART_2_TX_SCL_MISO_PIN_INDEX]);

        UART_2_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[UART_2_TX_SCL_MISO_PIN_INDEX]);

    #if (!UART_2_CY_SCBIP_V1)
        UART_2_SET_INP_DIS(UART_2_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     UART_2_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & UART_2_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!UART_2_CY_SCBIP_V1) */
    #endif /* (UART_2_RX_SDA_MOSI_PIN) */
